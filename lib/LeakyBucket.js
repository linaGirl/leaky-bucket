!function() {
    ...
    module.exports = new Class({
        ...
        , pause: function(howLong) {
            ...
            if (debug) log.info('Adding pause item with a cost of %s to the queue ...', cost);
        ...
        , reAdd: function(cost, cb) {
            ...
            if (debug) log.highlight('Re-Adding an item with a cost of %s to the beginning of the queue ...', cost);
            ...
            for (i = 1; i < this.queue.length; i++) {
                ...
                if ((totalCost+this.queue[i].cost)*this.refillRate > this.maxWaitingTime) {
                    item = this.queue.splice(i, 1)[0];

                    // reduce waiting time
                    this.waitTime -= item.cost;

                    if (debug) log.debug('Rejecting previosuly added item because its waiting time of %s s exceeds the max waiting time of %s s...', Math.ceil((totalCost+item.cost)*this.refillRate), this.maxWaitingTime);
            ...
            if (this.timer) {
                if (debug) log.warn('Canceled timer due to a re-add of an item ...');
            ...
            if (debug) log.info('Adding a pseudo item with a cost of 0 to the queue ...');
        ...
        , _throttle: function(cost, cb, fromQueue) {
            ...
            if (this.left >= cost && (!this.queue.length || fromQueue)) {
                ...
                if (waitTime > this.maxWaitingTime) {
                    if (debug) log.debug('Rejecting item because its waiting time of %s s exceeds the max waiting time of %s s...', waitTime, this.maxWaitingTime);
            ...
            if (this.queue.length && this.timer === null) {
                if (debug) log.debug('started a timer for the next item to be excuted, waiting %s milliseconds ...', Math.max(Math.ceil((this.queue[0].cost-this.left)*this.refillRate*1000), 0));
                ...
                this.timer = setTimeout(function() {
                    ...
                    else log.warn('Attempt to use non existent item from the queue');
